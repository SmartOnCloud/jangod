...
public class TemplateEngine {
	...
	String poolClass = application.getConfiguration().getProperty(
		"processor.pool");
	if (poolClass == null) {
	    pool = new ConcurrentListPool<Processor>();
	} else {
	    try {
		pool = (StatefulObjectPool<Processor>) Class.forName(poolClass)
			.newInstance();
	    } catch (Exception e) {
		pool = new ConcurrentListPool<Processor>();
		JangodLogger
			.warning("Can't instance processor pool(use default) >>> "
				+ poolClass);
	    }
	}
    }
    ...
    public String process(String templateFile, Map<String, Object> bindings,
    ...
    public void process(String templateFile, Map<String, Object> bindings,
