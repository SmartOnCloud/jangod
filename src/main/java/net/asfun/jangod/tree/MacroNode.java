...
public class MacroNode extends Node {
	...
	if (macro == null) {
	    throw new ParseException("Can't find macro >>> "
		    + master.getMacroName());
	}
	...
	if (tag != null) {
	    JangodLogger.fine("Treat macro as tag with same name >>> "
		    + master.getMacroName());
	    interpreter.setLevel(level);
	    return tag
		    .interpreter(children(), master.getHelpers(), interpreter);
	} else {
	    JangodLogger.warning("Skiping handless macro while rendering >>> "
		    + master.getMacroName());
