...
public class FilterParser implements Serializable {
    ...
    private String parseArg(String argString, List<String> args)
	    throws ParseException {
	...
	if (argString.charAt(0) == DQ) {
		...
		throw new ParseException(
			"Filter argument doesn't match quotes >>> "
				+ getVariable());
	    } else {
		// change to save quote in arg
	    ...
	    if (postComma == 0) {
		throw new ParseException("Filter lost some argument >>> "
			+ getVariable());
	    }
