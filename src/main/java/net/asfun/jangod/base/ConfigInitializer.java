...
public class ConfigInitializer {
	...
	if (configurationFile == null) {
		...
		file = null;
		JangodLogger.finest("Can't get file name from env by key >>> "
	...
	if (u != null) {
		...
		if (properties.containsKey("lib.imports")) {
			...
			try {
			    Importable importee = (Importable) Class.forName(
				    importClass).newInstance();
			    Configuration.addImport(importee);
			} catch (Exception e) {
			    JangodLogger
				    .warning("Can't created importable object >>> "
					    + importClass);
			}
		    }
		}
		...
		if (properties.containsKey("language")) {
		    try {
			config.setLocale(new Locale(properties
				.getProperty("language")));
		    } catch (Exception e) {
			config.setLocale(Locale.CHINESE);
			JangodLogger
				.warning("Language value from config file is invalid(use default) >>> "
		...
		if (properties.containsKey("timezone")) {
		    try {
			config.setTimezone(TimeZone.getTimeZone(properties
				.getProperty("timezone")));
		    } catch (Exception e) {
			config.setTimezone(TimeZone.getDefault());
			JangodLogger
				.warning("Timezone value from config file is invalid(use default) >>> "
		...
		JangodLogger.warning("Reading configuration file error >>> "
			+ e.getMessage());
	    } finally {
		if (is != null) {
		    try {
			...
			JangodLogger
				.warning("Reading configuration file error >>> "
					+ e.getMessage());
		    }
		}
	    }
