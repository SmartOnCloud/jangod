...
public class ResourceManager {
	...
	String locaterClass = config.getProperty("file.locater",
		"net.asfun.jangod.base.FileLocater");
	try {
	    locater = (ResourceLocater) Class.forName(locaterClass)
		    .newInstance();
	} catch (Exception e) {
	    locater = new FileLocater();
	    JangodLogger.warning("Can't instance file loader(use default) >>> "
		    + locaterClass);
	}
	...
	if (storeClass == null) {
	    ...
	    try {
		cache = (StatelessObjectStorage<String, String>) Class.forName(
			storeClass).newInstance();
	    } catch (Exception e) {
		cache = new SynchronousStorage<String, String>();
		JangodLogger
			.warning("Can't instance file cacher(use default) >>> "
				+ storeClass);
	    }
	}
    }

    public static String getResource(String file, String encoding)
	    throws IOException {
    ...
    public static String getFullName(String relativeName, String relativeDir,
	    String defaultDir) throws IOException {
	return locater.getFullName(relativeName, relativeDir, defaultDir);
    }

    public static String getFullName(String relativeName, String defaultDir)
	    throws IOException {
